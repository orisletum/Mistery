...
namespace thelab.mvc {
    ...
    public class TriggerView<T> : ColliderView<T> where T : BaseApplication {
		...
		void OnTriggerEnter(Collider p_collider) { if(enter) Notify(notification + "@trigger.enter",p_collider.gameObject,this.gameObject); }
		void OnTriggerExit(Collider p_collider)  { if(exit)  Notify(notification + "@trigger.exit", p_collider.gameObject); }
		void OnTriggerStay(Collider p_collider)  { if(stay)  Notify(notification + "@trigger.stay", p_collider.gameObject); }
    }
